...
-(Texture2D*) addImage: (NSString*) path
{
   ...
   twlogif(![[NSFileManager defaultManager] fileExistsAtPath:fullpath], "Missing image file:%@", [fullpath lastPathComponent]);

	// all images are handled by UIImage except PVR extension that is handled by our own handler
	if ( [[path lowercaseString] hasSuffix:@".pvr"] )
		return [self addPVRTCImage:fullpath];
	
   UIImage *fileImage = [UIImage imageWithContentsOfFile: fullpath ];
   twcheck(fileImage);
	tex = [ [Texture2D alloc] initWithImage:fileImage ];
   twcheck(tex);
