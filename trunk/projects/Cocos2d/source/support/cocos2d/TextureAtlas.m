...
{
	NSAssert( n < capacity_, @"updateColorWithQuadColor: Invalid index");
...
-(void) insertQuadWithTexture:(ccQuad2*)texCoords vertexQuad:(ccQuad3*)vertexCoords atIndex:(NSUInteger)idx
{
	NSAssert(idx < capacity_, @"updateQuadWithTexture: Invalid index");
	
	totalQuads_++;
	
	NSUInteger remaining = (totalQuads_-1) - idx;
	
	// last object doesn't need to be moved
	if( remaining ) {
		// tex coordinates
		memmove( &texCoordinates[idx+1],&texCoordinates[idx], sizeof(texCoordinates[0]) * remaining );
		// vertexCoordinates
		memmove( &vertexCoordinates[idx+1], &vertexCoordinates[idx], sizeof(vertexCoordinates[0]) * remaining );
		// colors
		if(withColorArray_)
			memmove(&colors[(idx+1)*4], &colors[idx*4], sizeof(colors[0]) * remaining * 4);
	}
	
	texCoordinates[idx] = *texCoords;
	vertexCoordinates[idx] = *vertexCoords;
}
...
-(void) removeQuadAtIndex:(NSUInteger) idx
{
	NSAssert(idx < totalQuads_, @"removeQuadAtIndex: Invalid index");
	...
	if( remaining ) {
		// tex coordinates
		memmove( &texCoordinates[idx],&texCoordinates[idx+1], sizeof(texCoordinates[0]) * remaining );
		// vertexCoordinates
		memmove( &vertexCoordinates[idx], &vertexCoordinates[idx+1], sizeof(vertexCoordinates[0]) * remaining );
		// colors
		if(withColorArray_)
			memmove(&colors[idx*4], &colors[(idx+1)*4], sizeof(colors[0]) * remaining * 4);
	}
